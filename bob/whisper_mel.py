"""Numpy-only Whisper-style log-mel features for Smart Turn v3.

Vendored from Pipecat's `_whisper_features` (BSD-2-Clause), which mirrors
`transformers.WhisperFeatureExtractor` so we do not import transformers.
"""
...
def _power_spectrogram(waveform: np.ndarray, window: np.ndarray, frame_length: int, hop_length: int) -> np.ndarray:
    pad = frame_length // 2
...
def compute_whisper_log_mel_features(audio: np.ndarray, *, do_normalize: bool = True) -> np.ndarray:
    """Return float32 log-mel of shape (80, 800) from 16 kHz mono audio."""
    if audio.ndim != 1:
